...
type Props = {
  /** Sabit yatay konum (px, merkez). */
  x: number;
  /** Dikey konum — oyun döngüsünden sürülen shared value (px, merkez). */
  ySV: SharedValue<number>;
  /** Velocity'den türetilen tilt açısı — shared value (radyan). */
 ...
 * Konum/tilt artık shared value ile sürülür → kare-başına React render yok,
 * tüm hareket UI thread'inde akıcı çalışır.
 */
function JumpCharacterBase({ x, ySV, tiltSV, size = 34, jumpTick }: Props) {
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    transform: [
      { translateY: ySV.value - size / 2 },
      { rotate: `${tiltSV.value}rad` },
      { scaleX: scaleX.value },
      { scaleY: scaleY.value },
    ],
  }));
...
export const JumpCharacter = memo(JumpCharacterBase);
