...
export default function TeacherSettings() {
  ...
  const prefs = profile?.teacherNotifPrefs ?? {};
  const [notif, setNotif] = useState(profile?.notificationsEnabled !== false);
  const [assignSub, setAssignSub] = useState(prefs.assignmentSubmissions !== false);
  const [questionSub, setQuestionSub] = useState(prefs.questionSubmissions !== false);
  ...
  const saveTeacherPref = async (
    key: 'assignmentSubmissions' | 'questionSubmissions',
    val: boolean,
  ) => {
    const uid = auth.currentUser?.uid;
    if (!uid) return;
    try {
      await updateDoc(doc(db, 'users', uid), { [`teacherNotifPrefs.${key}`]: val });
    } catch (err) {
      Alert.alert('Hata', (err as Error).message);
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-bg-base" edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 60 }}>
        ...
        <SettingsSection title="Bildirimler">
          ...
          <SettingsRow
            icon={ClipboardCheck}
            label="Ödev teslimleri"
            sublabel="Öğrenci ödev gönderince bildir"
            right={
              <Switch
                value={assignSub}
                onValueChange={(v) => {
                  setAssignSub(v);
                  saveTeacherPref('assignmentSubmissions', v);
                }}
              />
            }
          />
          <SettingsRow
            icon={FilePlus2}
            label="Soru gönderimleri"
            sublabel="Öğrenci/sisteme soru eklenince bildir"
            right={
              <Switch
                value={questionSub}
                onValueChange={(v) => {
                  setQuestionSub(v);
                  saveTeacherPref('questionSubmissions', v);
                }}
              />
            }
          />
      ...
      <EditProfileSheet
        ...
        initialBranch={(profile?.branch as string | undefined) ?? null}
      />
